# # 1b) Honeycomb Holstein Model with MPI Parallelization
# This tutorial will build on the previous [1a) Honeycomb Holstein Model](@ref) tutorial, demonstrating
...
# Now when initializing the [`SmoQyDQMC.SimulationInfo`](@extref) type, we also need to include the
...
# We also the [`SmoQyDQMC.initialize_datafolder`](@extref) function such that it takes the `comm` as the
...
# The main change we need to make from the previous [1a) Honeycomb Holstein Model](@ref) tutorial is to call
# the [`SmoQyDQMC.process_measurements`](@extref) and [`SmoQyDQMC.compute_composite_correlation_ratio`](@extref) functions
